...
pub(crate) use slice_ext_inner::SliceExt;

#[allow(dead_code)]
pub(crate) mod slice_ext_inner {
    ...
    impl<T> SliceExt for [T] {
        ...
        fn get_array<const ARRAY_LEN: usize>(
            ...
            self.get(offset..(offset + ARRAY_LEN)).map(|slice| {
                slice.try_into().expect(
                    "the arguments to `get` evaluate to the same length the return type uses",
                )
            })
        }
        ...
        fn split_first_chunk<const ARRAY_LEN: usize>(
            ...
            Some((
                first.try_into().expect("we're passing `ARRAY_LEN` to `split_at` above"),
                remainder,
            ))
        }
        ...
        fn split_last_chunk<const ARRAY_LEN: usize>(
            ...
            Some((
                remainder,
                last.try_into()
                    .expect("we're passing `self.len() - ARRAY_LEN` to `split_at` above"),
            ))
        }
    }
