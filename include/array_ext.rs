...
pub(crate) use array_ext_inner::ArrayExt;

#[allow(dead_code)]
pub(crate) mod array_ext_inner {
    ...
    pub(crate) trait ArrayExt {
        ...
        ///
        /// Note that unlike slicing where the second number is the end index, here the second
        ...
        /// returns `Option`. The rationale is that given the known length of the array, we always
        /// know that this will not return `None` so trying to keep the `std` method around is
        /// pointless. Importing the trait will also cause compile failures - that's also
        /// intentional to expose the places where useless checks are made.
        ...
        ///
        /// This is almost equivalent to just calling [`sub_array`](Self::sub_array) twice, except
        /// it also checks that the arrays don't overlap and that they cover the full range. This
        /// is very useful for demonstrating correctness, especially when chained. Using this
        /// technique even revealed a bug in the past.
        /// ([#4195](https://github.com/rust-bitcoin/rust-bitcoin/issues/4195))
        ...
        /// Fails to compile if the array is empty.
        ///
        /// Note that this method's name intentionally shadows the `std`'s `split_first` method
        /// which returns `Option`. The rationale is that given the known length of the array, we
        /// always know that this will not return `None` so trying to keep the `std` method around
        /// is pointless. Importing the trait will also cause compile failures - that's also
        /// intentional to expose the places where useless checks are made.
        ...
        /// Fails to compile if the array is empty.
        ///
        /// Note that this method's name intentionally shadows the `std`'s `split_last` method
        /// which returns `Option`. The rationale is that given the known length of the array, we
        /// always know that this will not return `None` so trying to keep the `std` method around
        /// is pointless. Importing the trait will also cause compile failures - that's also
        /// intentional to expose the places where useless checks are made.
        ///
        /// The returned tuple is also reversed just as `std` for consistency and simpler diffs
        /// when migrating.
